...
def load_resource(url_or_path: str) -> bytes:
    """Load data from from url or path."""
    if url_or_path.startswith("s3://"):
        s3 = boto3.resource("s3", region_name='us-east-2',
                            aws_access_key_id=config('AWS_ACCESS_KEY_ID', None),
                            aws_secret_access_key=config('AWS_SECRET_ACCESS_KEY', None))
        bucket, key = url_or_path[5:].split("/", 1)
        return s3.Object(bucket, key).get()["Body"].read()
    elif url_or_path.startswith("https://") or url_or_path.startswith("http://"):
        return request.urlopen(url_or_path).read()
    elif os.path.exists(url_or_path):
        with open(url_or_path, "rb") as fp:
            return fp.read()
    else:
        raise ValueError(f"Unknown resource: {url_or_path}")
