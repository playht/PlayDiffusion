...
class RotaryPositionalEmbeddings(nn.Module):
    ...
    def forward(self, x: Tensor, input_pos: Optional[Tensor] = None) -> Tensor:
        ...
        # During batched inference, input_pos will differ across batch dimension due to left padding
        bs = 1 if (input_pos is None) or (input_pos.ndim <= 1) else input_pos.shape[0]
        # reshape the cache for broadcasting
        # tensor has shape [1, s, 1, n_d // 2, 2]
        rope_cache = rope_cache.view(bs, xshaped.size(1), 1, xshaped.size(3), 2)
