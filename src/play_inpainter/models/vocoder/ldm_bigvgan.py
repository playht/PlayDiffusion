...
def load_ldm_bigvgan(
        ...
        batch_vocoder_size: int = 1,
        concurrency_limit: Optional[int] = None,
        dtype: torch.dtype = torch.float16,
        device: torch.device = torch.device("cuda")
    ) -> BigVGAN:

    if checkpoint.endswith(".st"):
        h = load_model_params_st(checkpoint)
        state = safetensors.torch.load_file(checkpoint)
    ...
    if concurrency_limit is not None:
        h["concurrency_limit"] = concurrency_limit
...
class Upsampler(nn.Module):
    def __init__(self, h):
        ...
        self.in_channels = h.ar_tokens_dim + h.speaker_cond_dim
        self.mel_channel = h.num_mels
        self.num_upsamples = h.ar_tokens_hop_size // (2*h.hop_size)

        num_channels_prev_mel = 256
        ...
        for i in range(self.num_upsamples):
            self.ups.append(
                ...
                padding=stride // 2 + stride % 2,
                output_padding=stride % 2)))
            self.ups.append(nn.LeakyReLU(LRELU_SLOPE))

        conv_channels = in_channels // (2 ** (self.num_upsamples) // 2 )
        self.post = nn.Sequential(
            weight_norm(Conv1d(conv_channels, num_channels_prev_mel, 7, 1, padding=3)),
            nn.LeakyReLU(LRELU_SLOPE),
            weight_norm(Conv1d(num_channels_prev_mel, self.mel_channel, 7, 1, padding=3)),
            nn.LeakyReLU(LRELU_SLOPE))
    ...
    def remove_weight_norm(self):
        print('Removing weight norm from upsampler...')
...
class AMPBlock2(torch.nn.Module):
    def __init__(self, h, channels, kernel_size=3, dilation=(1, 3), activation=None, bigvgan_kernels=None):
...
class EmbeddingLayer(torch.nn.Module):
    def __init__(self, h):
        ...
        self.embedding = nn.Embedding(h.codes_vocab_size, h.ar_tokens_dim)
...
class BigVGAN(torch.nn.Module):
    def __init__(self, h):
        ...
        concurrency_limit = h.get('concurrency_limit', None)
        if concurrency_limit is not None:
            self.concurrency_limit_semaphore = threading.Semaphore(concurrency_limit)
        else:
            self.concurrency_limit_semaphore = None
        ...
        for i in range(len(self.ups)):
            ...
            for j, (k, d) in enumerate(zip(h.resblock_kernel_sizes, h.resblock_dilation_sizes)):
                self.resblocks.append(resblock(h, ch, k, d, activation=h.activation, bigvgan_kernels=self.bigvgan_kernels))
        ...
        # whether to use XLSR clusters rather than embedding layer
        self.use_xlsr_centroids = h.get('use_xlsr_centroids', False)

        if h.get("from_codes", False):
            if not self.use_xlsr_centroids:
                self.embedding_layer = EmbeddingLayer(h)
    ...
    def sample(self, text, ar_h, voice_emb):
        if self.concurrency_limit_semaphore is not None:
            with self.concurrency_limit_semaphore:
    ...
    @torch.inference_mode()
    def sample_stream(self, text, stream, voice_emb, size_overlap, start_of_request):
        if self.concurrency_limit_semaphore is not None:
            with self.concurrency_limit_semaphore:
                return self.sample_stream_impl(stream, voice_emb, size_overlap)
        else:
            return self.sample_stream_impl(stream, voice_emb, size_overlap)

    @torch.inference_mode()
    def sample_stream_impl(self, stream, voice_emb, size_overlap):
        if voice_emb.ndim == 3:
            voice_emb = voice_emb.squeeze(1)

        overlap = int(self._ar_tokens_hop_size * size_overlap)

        hann_window = torch.hann_window(2*overlap+1, periodic=False).to(self.device)
        half_hann_left = hann_window[:overlap+1]
        half_hann_right = hann_window[overlap:]

        try:
            audio = None
            for units, latents in stream:
                t_start = time.perf_counter()
                if self.h.from_codes:
                    audio_piece = self.forward(units, voice_emb)[0].squeeze()
                else:
                    audio_piece = self.forward(latents, voice_emb)[0].squeeze()
                print(f"Vocoder time: {(time.perf_counter() - t_start) * 1000:.1f} ms")

                if audio is None:
                    # Force the initial, un-adjusted output to fp32 to match the rest of the output.
                    audio = audio_piece.to(torch.float32)
                    yield (units[0, :-size_overlap],
                           latents[..., :-size_overlap] if latents is not None else None,
                           audio[:-overlap])
                    continue

                # Handle the case where the audio_piece is shorter than the overlap.
                cur_overlap = min(overlap, len(audio_piece), len(audio))

                # NOTE: for now we are just using a crude OLA at the last part of the segment.
                # this will be improved later
                left_overlap = audio[-cur_overlap:] * half_hann_right[1:cur_overlap+1]
                right_overlap = audio_piece[:cur_overlap] * half_hann_left[1:cur_overlap+1]

                audio = torch.cat(
                    (left_overlap + right_overlap, audio_piece[cur_overlap:]),
                    dim=-1
                )
            ...
            if audio is not None and audio.any():
                yield (units[0, -size_overlap:],
                       latents[..., -size_overlap:] if latents is not None else None,
                       audio[-overlap:])
...
class BatchedVocoderWrapper:
    """
    A simple wrapper around BigVGAN vocoder that batches inference requests.
    Maintains the same API as the original vocoder but processes inputs in batches.
    """
    def __init__(self, vocoder, max_batch_size, max_wait_ms, max_chunk_size):
        """
        Args:
            vocoder: The original BigVGAN vocoder model
            max_batch_size: Maximum number of samples to process in a batch
            max_wait_ms: Maximum time to wait for batch to fill up (milliseconds)
        """
        ...
        self._active_requests_count_lock = threading.Lock()
    ...
    @torch.inference_mode()
    def sample(self, text, ar_h, voice_emb):
        return self.vocoder.sample(text, ar_h, voice_emb)

    @torch.inference_mode()
    def sample_stream(self, text, stream, voice_emb, size_overlap, start_of_request=True):
        """
        Args:
            text: Text input (unused, kept for API compatibility)
            stream: Iterator yielding (units, latents) tuples
            voice_emb: Voice embedding tensor for conditioning
            size_overlap (float): Fraction of the chunk to use for overlapping
            start_of_request (bool): Whether this is the first chunk of a request
        Yields:
            Tuples of (units, latents, audio) with proper overlapping applied
        """
        if voice_emb.ndim == 3:
            voice_emb = voice_emb.squeeze(1)

        overlap = int(self.vocoder._ar_tokens_hop_size * size_overlap)
        ...
        try:
            with self._active_requests_count_lock:
                self._active_requests_count += 1

            audio = None
            for units, latents in stream:
                # Process through batch queue system (will be batched with other concurrent requests)
                request_id = str(uuid.uuid4())
                input_tensor = units if self.vocoder.h.from_codes else latents

                is_priority = audio is None
                self._batch_queue.put((request_id, input_tensor, voice_emb, is_priority))
                result = None
    ...
    def _batch_processor_loop(self):
        while not self._stop_event.is_set():
            try:
                ...
                try:
                    batch.append(self._batch_queue.get(timeout=0.1))
                except queue.Empty:
                    continue

                # Fill the batch with available requests
                ...
                wait_until_ms = start_time_ms + self.max_wait_ms
                while len(batch) < self.max_batch_size:
                    current_time_ms = time.time_ns() // 1_000_000
                    remaining_ms = max(0, wait_until_ms - current_time_ms)
                    if remaining_ms <= 0:
                        print(f"Batch processor timed out waiting for more requests")
                        break
                    if len(batch) >= self._active_requests_count:
                        print(f"Breaking batch processor loop due to active requests")
                        break
                    if any(item[-1] for item in batch):
                        print(f"Breaking batch processor loop due to priority request")
                        break

                    try:
                        batch.append(self._batch_queue.get(timeout=min(0.001, remaining_ms / 1000)))
                        while len(batch) < self.max_batch_size:
                            try:
                                batch.append(self._batch_queue.get_nowait())
                            except queue.Empty:
                ...
                ar_hs       = [item[1] for item in batch]
                voice_embs  = [item[2] for item in batch]
                # Pad all sequences to the same length
                max_seq_len = max(ar_h.shape[-1] for ar_h in ar_hs)
                max_seq_len = max(max_seq_len, self.max_chunk_size)
                padded_inputs = []
                orig_seq_lengths = []  # Store original lengths for unpadding later
                for ar_h in ar_hs:
                    seq_len = ar_h.shape[-1]
                    orig_seq_lengths.append(seq_len)
                    if seq_len == max_seq_len:
                        padded_inputs.append(ar_h)
                        continue

                    padding_mode = 'constant' if self.vocoder.h.from_codes else 'replicate'
                    if ar_h.ndim == 2:  # [batch, seq_len]
                        padded = torch.nn.functional.pad(ar_h, (0, max_seq_len - seq_len), mode=padding_mode)
                    elif ar_h.ndim == 3:  # [batch, channels, seq_len]
                        padded = torch.nn.functional.pad(ar_h, (0, max_seq_len - seq_len, 0, 0), mode=padding_mode)
                    else:
                        raise ValueError(f"Unexpected input dimension: {ar_h.ndim}")

                    padded_inputs.append(padded)

                # Ensure we have a full batch size by repeating the last input if needed
                # Repeat the last input to pad to max_batch_size
                actual_batch_size = len(padded_inputs)
                if actual_batch_size < self.max_batch_size:
                    last_input = padded_inputs[-1]
                    last_emb = voice_embs[-1]
                    for _ in range(self.max_batch_size - actual_batch_size):
                        padded_inputs.append(last_input.clone())
                        voice_embs.append(last_emb.clone())
                        # Add a dummy sequence length (doesn't matter as we won't use these results)
                        orig_seq_lengths.append(max_seq_len)

                stacked_input = torch.cat(padded_inputs, dim=0)
                stacked_embs  = torch.cat([emb.unsqueeze(0) if emb.ndim == 1 else emb for emb in voice_embs], dim=0)
                if stacked_embs.ndim == 3:
                    stacked_embs = stacked_embs.squeeze(1)

                self._cuda_stream.wait_stream(torch.cuda.current_stream())
                with torch.cuda.stream(self._cuda_stream):
                    batch_results = self.vocoder.forward(stacked_input, stacked_embs)
                    print(f"BatchedVocoder time: {(time.perf_counter() - t_start) * 1000:.1f} ms for {actual_batch_size}")
                torch.cuda.current_stream().wait_stream(self._cuda_stream)

                # Calculate audio length based on original sequence lengths
                # Using the vocoder's hop size to convert from feature frames to audio samples
                audio_lengths = [int(sl * self.vocoder._ar_tokens_hop_size) for sl in orig_seq_lengths[:actual_batch_size]]

                with self._batch_lock:
                    for i, request_id in enumerate(request_ids):
                        audio_len = audio_lengths[i]
                        if batch_results.ndim == 3:  # [batch, channels, audio_len]
                            result = batch_results[i:i+1, :, :audio_len]
                        elif batch_results.ndim == 2:  # [batch, audio_len]
                            result = batch_results[i:i+1, :audio_len]
                        else:
                            result = batch_results[i:i+1]  # Keep as is for unusual dimensions

                        self._results[request_id] = result
            except Exception as e:
                print(f"Error in Batched Vocoder: {str(e)}")
                if not batch:
                    continue
                request_ids = [item[0] for item in batch]
                with self._batch_lock:
                    for req_id in request_ids:
                        self._results[req_id] = e
    ...
    def remove_weight_norm(self):
        return self.vocoder.remove_weight_norm()
    ...
    @property
    def output_frequency(self):
        return self.vocoder._output_frequency
    ...
    @property
    def device(self):
        return self.vocoder.conv_pre.weight.device

    @property
    def dtype(self):
        return self.vocoder.conv_pre.weight.dtype
